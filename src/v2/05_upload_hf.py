#!/usr/bin/env annotation
#!/usr/bin/env python3
"""Helper script to generate README.md, copy plots, and upload 1B ONNX model directories to Hugging Face Hub - V2."""
...
# Fallback sizes if directories not found (for 1B models)
...
def load_direction_benchmarks(direction: str) -> dict[str, dict]:
    """Load all benchmark json files for a given direction (1B version)."""
    benchmarks = {}
    for p in ["fp16", "int8", "q4f16"]:
        path = Path(f"fixtures/benchmark-{direction}-1b-{p}.json")
        if path.exists():
            try:
...
def generate_benchmark_markdown(direction: str, precision: str, benchmarks: dict[str, dict]) -> str:
    ...
    size_fp32 = MODEL_SIZES_FALLBACK.get(direction, {}).get("fp32", 6800.0)
    ...
    if precision in benchmarks:
        ...
        # Mismatches list
        mismatches_path = Path(f"fixtures/benchmark-{direction}-1b-{precision}-mismatches.json")
...
def generate_default_readme(direction: str, repo_id: str, precision: str) -> str:
    """Generate default README content matching the model's direction and precision (1B models)."""
    indic_languages = [
    ...
    title = f"IndicTrans2 1B ({direction_title}) — ONNX bundle [{precision_label}]"
    ...
    base_name = base_repos.get(direction, "indictrans2-en-indic-1B-ONNX")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Upload 1B ONNX model bundle to Hugging Face Hub - V2")
    ...
    # Copy plots from fixtures to the upload directory if they exist (using 1B plots prefix)
    plot_prefix = f"{direction.replace('-', '_')}_1b"
    ...
    # Copy translate.py into the bundle
    translate_src = Path(__file__).parent.parent / "translate.py"
