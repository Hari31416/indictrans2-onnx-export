...
def write_markdown_report(data):
    ...
    content.append("")
    content.append("- **WASM CPU FP32 Supremacy on Base**: On the Base model (200M/320M), **WASM CPU FP32 achieved a blazing 102.5 tokens/sec**, outperforming all other options (including WebGPU FP32 at 15.2 t/s). At smaller scales, CPU execution via native WebAssembly 128-bit SIMD completely bypasses the GPU command submission and CPU-GPU transfer overhead that bottleneck WebGPU.")
    content.append("- **WASM CPU FP16 Software Emulation Trap**: On the CPU, WASM FP16 drops to only **14.1 tokens/sec** (a 7.2x degradation compared to FP32). Browser CPUs lack native hardware support for half-precision math, forcing the runtime into slow software emulation.")
    content.append("- **Optimized WASM CPU INT8 Execution**: WASM CPU INT8 runs at **62.8 tokens/sec** using native integer SIMD hardware instructions (Intel VNNI / ARM NEON dot-product).")
    content.append("- **1B Scale WebGPU Mandatory Acceleration**: For the 1B scale model, WebGPU Q4F16 is accelerated to **16.3 tokens/sec**, whereas WASM CPU degrades to an unusable **2.4 tokens/sec** (a **6.8x speedup** for WebGPU). At 1B parameters, compute density far outweighs GPU bus latency, making GPU acceleration mandatory.")
    content.append("")
    ...
    content.append("")
    content.append("- **WASM CPU TTFT (Prefill) Penalty**: Time to First Token on WASM CPU for the 1B Q4F16 model is **1014 ms**, compared to only **224 ms** on WebGPU. Prompt processing latency on WASM CPU introduces a visible 1-second delay, while WebGPU remains highly responsive.")
    content.append("- **Step Latency Comparison**: During autoregressive decoding, WebGPU runs at **77-100 ms per token** for the 1B model, whereas WASM CPU takes **461 ms per token** on 1B Q4F16, causing character-by-character lagging.")
    content.append("")
